...
async function newPage(context, onlyHTML) {
 ...
 */
exports.getBrowserPage = async function getBrowserPage(
  browser,
  contextId,
  pageId,
  onlyHTML
  ...
  if (contextId && pageId) {
    ...
    let context = await findContextInBrowser(browser, contextId);
    return await newPage(context, onlyHTML);
  } else {
...
exports.performAction = async function performAction(request, action) {
  const { contextId, pageId, onlyHTML } = request.query;
  const lock = request.app.get("lock");
  const page = await exports.getBrowserPage(
    request.app.get("browser"),
    contextId,
    pageId,
    onlyHTML
  );
